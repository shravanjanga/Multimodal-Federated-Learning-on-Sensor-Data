...
class Server:
    ...
    def eval(self, data_test):
        class_correct = dict() #per-class
        class_total = dict()   #per-class
        ...
        win_loss = []
        win_accuracy = []
        win_f1 = []  # Initialize the list for weighted F1 scores
        ...
        for i in range(n_eval_process):
            ...
            win_f1.append(weighted_f1)  # Append the calculated F1 score
        ...
        per_class_accuracy = {cls: correct / class_total[cls]
                              for cls, correct in class_correct.items()}
            
            

        #return np.mean(win_loss), np.mean(win_accuracy), np.mean(win_f1)  # Use win_f1 here
        return np.mean(win_loss), np.mean(win_accuracy), np.mean(win_f1), per_class_accuracy
        
####
    def write_result(self, round_number, loss, accuracy, f1_score, per_class_accuracy, file_path):
        """Writes simulation results to a file.

        Args:
            round_number: Current round number.
            loss: Loss obtained in this round.
            accuracy: Accuracy obtained in this round.
            f1_score: F1 score obtained in this round.
            per_class_accuracy: Dictionary of per-class accuracies.
            file_path: Path to the file where results will be saved.
        """
        with open(file_path, 'a') as file:
            file.write(f"Round: {round_number}, Loss: {loss:.4f}, Accuracy: {accuracy:.4f}, F1 Score: {f1_score:.4f}\n")
            file.write("Per-Class Accuracy:\n")
            for class_label, class_acc in per_class_accuracy.items():
                file.write(f"\tClass {class_label}: {class_acc:.4f}\n")
